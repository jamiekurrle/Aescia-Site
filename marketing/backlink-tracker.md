...
| 3 | ClinicalTrials.gov / ANZCTR | https://anzctr.org.au/Trial/Registration/TrialReview.aspx?ACTRN=12625001425482 (registered); https://clinicaltrials.gov (not yet registered) | requested | (ANZCTR registered prior to this tracker) | (ANZCTR live) | SAFE-Discharge is registered on ANZCTR. Confirm whether dual-registration on ClinicalTrials.gov is wanted; if so, register and link from /evidence. ANZCTR record already linked via Organization sameAs in lib/schema.ts. |
| 4 | MTAA member directory | https://www.mtaa.org.au (member directory; verify exact URL post-Compass enrollment) | not started | — | — | Request listing in the MTAA member directory after Compass onboarding completes. Send company info from `wellfound-crunchbase.md`, logo, contact. Confirm whether MTAA membership and Compass enrollment imply directory listing automatically. |
