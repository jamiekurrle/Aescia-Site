...
export const dict: Record<string, Record<string, string>> = {
  en: {
    ...
    'roi.assumptions.endoscopistFee': 'Endoscopist professional fee: the per-scope figure you set drives the separate endoscopist loss line, on the same recoverable slots. Default $300 is a commercial-basis physician fee, chosen to match the commercial facility default: the Medicare ASC professional fee for a colonoscopy is ~$215, marked up at a commercial physician rate (MedPAC/CBO put commercial physician pay at ~1.3–1.5× Medicare). Medicare-heavy lists collect closer to $215; set this to your own payer mix.',
  ...
  fr: {
    ...
    'roi.field.endoscopistFee.hint': 'Les honoraires de l\'opérateur par scope, distincts des frais d\'établissement ci-dessus. La valeur par défaut $300 est un honoraire médical sur base commerciale (honoraire professionnel ASC Medicare ~$215 majoré à un tarif médical commercial), en cohérence avec la valeur par défaut de l\'établissement commercial ; les listes à forte composante Medicare encaissent plus près de $215. Détermine seulement la ligne de perte de l\'endoscopiste, pas le multiple ROI.',
    ...
    'roi.assumptions.endoscopistFee': 'Honoraires professionnels de l\'endoscopiste : le chiffre par scope que vous fixez détermine la ligne de perte distincte de l\'endoscopiste, sur les mêmes créneaux récupérables. La valeur par défaut $300 est un honoraire médical sur base commerciale, choisie pour correspondre à la valeur par défaut de l\'établissement commercial : l\'honoraire professionnel ASC Medicare pour une coloscopie est d\'environ $215, majoré à un tarif médical commercial (MedPAC/CBO situent la rémunération médicale commerciale à environ 1,3–1,5× Medicare). Les listes à forte composante Medicare encaissent plus près de $215 ; réglez selon votre propre composition de payeurs.',
  ...
  es: {
    ...
    'roi.field.endoscopistFee.hint': 'Los honorarios del operador por scope, distintos de la tarifa de instalación anterior. El valor predeterminado $300 es un honorario médico de base comercial (honorario profesional ASC de Medicare ~$215 incrementado a una tarifa médica comercial), que coincide con el valor predeterminado de la instalación comercial; las listas con alta proporción de Medicare cobran más cerca de $215. Determina solo la línea de pérdida del endoscopista, no el múltiplo de ROI.',
    ...
    'roi.assumptions.endoscopistFee': 'Honorarios profesionales del endoscopista: la cifra por scope que usted fija determina la línea de pérdida aparte del endoscopista, sobre los mismos espacios recuperables. El valor predeterminado $300 es un honorario médico de base comercial, elegido para coincidir con el valor predeterminado de la instalación comercial: el honorario profesional ASC de Medicare para una colonoscopia es ~$215, incrementado a una tarifa médica comercial (MedPAC/CBO sitúan la remuneración médica comercial en ~1,3–1,5× Medicare). Las listas con alta proporción de Medicare cobran más cerca de $215; ajústelo a su propia combinación de pagadores.',
  ...
  zh: {
    ...
    'roi.field.endoscopistFee.hint': '操作医师每个 scope 的费用，与上面的设施费不同。默认值 $300 是一项商业基准的医师费用（Medicare ASC 专业费约 $215，上调至商业医师费率），与商业设施默认值相匹配；Medicare 占比高的清单收取更接近 $215。仅影响内镜医师损失行，不影响 ROI 倍数。',
    ...
    'roi.assumptions.endoscopistFee': '内镜医师专业费：您设置的每个 scope 数字决定单独的内镜医师损失行，针对同样的可回收时段。默认值 $300 是一项商业基准的医师费用，选用它是为了与商业设施默认值相匹配：一次结肠镜检查的 Medicare ASC 专业费约为 $215，上调至商业医师费率（MedPAC/CBO 将商业医师报酬定为 Medicare 的约 1.3–1.5×）。Medicare 占比高的清单收取更接近 $215；请按您自己的付款方构成设置。',
  ...
  ar: {
    ...
    'roi.field.endoscopistFee.hint': 'أجر الطبيب المُجري لكل scope، منفصل عن رسوم المنشأة أعلاه. القيمة الافتراضية $300 هي أجر طبيب على أساس تجاري (الأجر المهني لـ ASC في Medicare ~$215 مزاد عليه بمعدّل طبيب تجاري)، بما يطابق القيمة الافتراضية للمنشأة التجارية؛ والقوائم ذات النسبة العالية من Medicare تحصّل أقرب إلى $215. ويؤثّر فقط في سطر خسارة طبيب التنظير، لا في مضاعِف ROI.',
    ...
    'roi.assumptions.endoscopistFee': 'الأجر المهني لطبيب التنظير: الرقم لكل scope الذي تحدّده يقود سطر خسارة طبيب التنظير المنفصل، على المواعيد القابلة للاسترداد نفسها. القيمة الافتراضية $300 هي أجر طبيب على أساس تجاري، اختير ليطابق القيمة الافتراضية للمنشأة التجارية: فالأجر المهني لـ ASC في Medicare لتنظير القولون نحو $215، مزاد عليه بمعدّل طبيب تجاري (يقدّر MedPAC/CBO أجر الطبيب التجاري بنحو 1.3–1.5× Medicare). والقوائم ذات النسبة العالية من Medicare تحصّل أقرب إلى $215؛ اضبط هذا بحسب مزيج الدافعين الخاص بك.',
  ...
  vi: {
    ...
    'roi.field.endoscopistFee.hint': 'Phí của bác sĩ thực hiện cho mỗi scope, tách biệt với phí cơ sở ở trên. Mặc định $300 là phí bác sĩ trên cơ sở thương mại (phí chuyên môn ASC của Medicare ~$215 được nâng lên mức phí bác sĩ thương mại), khớp với giá trị mặc định của cơ sở thương mại; các danh sách nặng Medicare thu gần $215 hơn. Chỉ tác động đến dòng tổn thất của bác sĩ nội soi, không tác động đến bội số ROI.',
    ...
    'roi.assumptions.endoscopistFee': 'Phí chuyên môn của bác sĩ nội soi: con số cho mỗi scope mà bạn đặt sẽ điều khiển dòng tổn thất riêng của bác sĩ nội soi, trên cùng những suất có thể phục hồi. Mặc định $300 là phí bác sĩ trên cơ sở thương mại, được chọn để khớp với giá trị mặc định của cơ sở thương mại: phí chuyên môn ASC của Medicare cho một ca nội soi đại tràng là ~$215, được nâng lên mức phí bác sĩ thương mại (MedPAC/CBO đặt mức thù lao bác sĩ thương mại ở khoảng 1,3–1,5× Medicare). Các danh sách nặng Medicare thu gần $215 hơn; hãy đặt theo cơ cấu bên chi trả của riêng bạn.',
