...
/**
 * EditorialHero with liquid-glass on the primary CTA. Tuned so it reads as
 * refined surgical-instrument chrome, not toy plastic: low aberration, low
 * displacement, low elasticity.
 */
export function EditorialHeroGlass() {
  const { t } = useI18n()
  ...
  return (
    <section
      ref={heroRef}
      className="relative overflow-hidden bg-foreground text-background pt-28 lg:pt-32 pb-32 lg:pb-44"
    >
      <div
      ...
      <div className="relative w-full max-w-7xl mx-auto px-6 lg:px-10 grid lg:grid-cols-12 gap-12 lg:gap-16 items-center min-h-[calc(100vh-240px)] lg:min-h-[calc(100vh-220px)]">
        <div className="lg:col-span-7 pt-8 lg:pt-12">
          ...
          <div className="mt-10 lg:mt-12 flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-8">
            <Link href="/platform" className="inline-block" aria-label={t('hero.cta.primary')}>
              <LiquidGlass
                ...
                mode="standard"
              >
                <span className="inline-flex items-center text-[14px] font-medium tracking-wide text-background">
                  {t('hero.cta.primary')}
