...
/**
 * EditorialHero with a live ShaderGradient v2 replacing the static radial gradient.
 * Same layout, same copy, same diagram — only the atmosphere changes.
 */
export function EditorialHeroGradient() {
  const { t } = useI18n()
  ...
  return (
    <section className="relative overflow-hidden bg-foreground text-background pt-28 lg:pt-32 pb-32 lg:pb-44">
      {/* Live shader gradient, tuned slow + low contrast so it reads as atmosphere.
          Rendered as a direct child of <section> so ShaderGradientCanvas can
          size against the section's relative box. */}
      {mounted && <ShaderBg preset="calm-ink" />}
      {/* Mute the gradient so text contrast stays clinical */}
      <div className="absolute inset-0 bg-foreground/40 pointer-events-none" aria-hidden="true" />
      {/* Brass kiss at top right, identical to the original static gradient */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: 'radial-gradient(ellipse 90% 60% at 85% 15%, oklch(0.73 0.09 80 / 0.10), transparent 55%)',
        }}
        aria-hidden="true"
      />
