...
export default function HospitalsPage() {
  ...
  const complications = [1, 2, 3, 4, 5, 6].map((n) => ({
    ...
    <main className="bg-background min-h-screen">
      ...
      <section className="pt-40 pb-24 lg:pt-48 lg:pb-28 px-6 lg:px-10 border-b border-border">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 lg:pt-8">
            <div className="border-l border-brass/40 pl-6 py-2">
              <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-brass mb-3">Status</p>
              ...
              <p className="text-[13px] text-foreground/60 leading-relaxed">
                Aescia for Hospitals is on a Class IIa medical-device pathway. It is not yet listed for commercial deployment. Engagement today is through evaluation and pilot contracts only.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 lg:py-32 px-6 lg:px-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-border">
            {complications.map((c) => (
              <div key={c.n} className="bg-background p-8 lg:p-10">
                <div className="flex items-center gap-3 mb-6">
                  <span className="font-mono text-[11px] text-brass tracking-widest">{c.n}</span>
                  <span className="h-px w-6 bg-brass/50" />
                </div>
                <h3
                  className="font-display text-[20px] lg:text-[22px] leading-[1.2] tracking-[-0.015em] mb-3.5"
      ...
      <section className="py-24 lg:py-32 px-6 lg:px-10 bg-secondary">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-12">
          ...
          {/* Stylised worklist preview, not a product screenshot */}
          <div className="lg:col-span-7">
            <div className="bg-background border border-border shadow-[0_1px_0_0_rgba(0,0,0,0.03)] overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-secondary">
                <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-foreground/55">Discharge cohort · Mon 14 Apr</span>
                <span className="font-mono text-[11px] text-foreground/40">42 active</span>
              </div>
              <ul className="divide-y divide-border">
                {[
                  ...
                  <li key={i} className="grid grid-cols-[40px_1fr_auto] items-center gap-4 px-5 py-4">
                    <div
                      className={`font-mono text-[12px] text-center py-1 font-medium ${
                        row.level >= 4
                          ? 'bg-destructive/15 text-destructive'
                          : row.level === 3
                          ? 'bg-brass/20 text-foreground'
                          : 'bg-secondary text-foreground/55'
                      }`}
                      title={`Urgency level ${row.level}`}
                    >
                      L{row.level}
                    </div>
                    <div>
                      <div className="text-[14px] text-foreground">{row.name}</div>
                      <div className="text-[12px] text-foreground/60 mt-0.5">{row.reason}</div>
                    </div>
                    <div className="font-mono text-[11px] text-foreground/45">{row.ago}</div>
                  </li>
                ))}
              </ul>
              <div className="px-5 py-3 border-t border-border bg-secondary flex items-center justify-between">
                <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-foreground/50">Illustrative preview</span>
                <span className="text-[11px] text-foreground/45">Live view is configured to your escalation policy</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-6 lg:col-start-7">
            <dl className="divide-y divide-border border-y border-border">
              <div className="grid grid-cols-[140px_1fr] gap-6 py-6">
                <dt className="font-mono text-[11px] uppercase tracking-[0.18em] text-foreground/50">Registration</dt>
                <dd className="font-mono text-[14px] text-foreground">{t('hospitals.trial.id')}</dd>
              </div>
              <div className="grid grid-cols-[140px_1fr] gap-6 py-6">
                <dt className="font-mono text-[11px] uppercase tracking-[0.18em] text-foreground/50">Site</dt>
                <dd className="text-[15px] text-foreground">{t('hospitals.trial.site')}</dd>
              </div>
              <div className="grid grid-cols-[140px_1fr] gap-6 py-6">
                <dt className="font-mono text-[11px] uppercase tracking-[0.18em] text-foreground/50">Unit</dt>
                <dd className="text-[15px] text-foreground">{t('hospitals.trial.unit')}</dd>
              </div>
              <div className="grid grid-cols-[140px_1fr] gap-6 py-6">
                <dt className="font-mono text-[11px] uppercase tracking-[0.18em] text-foreground/50">Patients</dt>
                <dd className="font-display text-[20px] text-foreground" style={{ fontVariationSettings: "'opsz' 72, 'SOFT' 30" }}>500, single centre</dd>
              </div>
              <div className="grid grid-cols-[140px_1fr] gap-6 py-6">
                <dt className="font-mono text-[11px] uppercase tracking-[0.18em] text-foreground/50">Sponsor</dt>
                <dd className="text-[15px] text-foreground">Royal Prince Alfred Hospital (via cardiothoracic department)</dd>
              </div>
              <div className="grid grid-cols-[140px_1fr] gap-6 py-6">
                <dt className="font-mono text-[11px] uppercase tracking-[0.18em] text-foreground/50">Status</dt>
                <dd className="text-[15px] text-foreground">Ethics and governance in final stages; first enrolments expected through 2026.</dd>
              </div>
            </dl>
          </div>
        </div>
      ...
      <section className="py-24 lg:py-32 px-6 lg:px-10 bg-foreground text-background">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-px bg-background/10">
            {path.map((p) => (
              <div key={p.n} className="bg-foreground p-8 lg:p-10">
                <div className="flex items-center gap-3 mb-8">
                ...
                <p className="text-[14px] leading-[1.7] text-background/70">
                  {p.desc}
                </p>
              </div>
            ))}
      ...
      <section className="py-24 lg:py-32 px-6 lg:px-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-[16px] lg:text-[17px] leading-[1.7] text-foreground/75 max-w-2xl mx-auto mb-10">
            {t('hospitals.integration.body')}
          </p>
          <div className="inline-flex flex-wrap justify-center gap-2">
            {['SAML 2.0', 'OIDC', 'HL7 v2 ADT', 'FHIR R4', 'ISO 27001', 'IEC 62304'].map((item) => (
              <span key={item} className="font-mono text-[11px] tracking-wide border border-border px-3 py-1.5 text-foreground/70">{item}</span>
            ))}
          </div>
        </div>
      </section>
